...
// encryption with IV
QByteArray MiniAES::Encrypt(QByteArray p_input, QByteArray p_key)
{
...
QByteArray MiniAES::Decrypt(QByteArray p_input, QByteArray p_key)
{
...
// basic encryption
QByteArray MiniAES::Encrypt(QByteArray p_input, QByteArray p_key, QByteArray p_iv)
{
...
QByteArray MiniAES::Decrypt(QByteArray p_input, QByteArray p_key, QByteArray p_iv)
{
...
// helper functions
QByteArray MiniAES::HexStringToByte(QString key)
{
    return QByteArray::fromHex(QString(key).toLatin1());
}

void MiniAES::QByteArrayToUCharArray(QByteArray src, unsigned char *dest)
{
...
QByteArray MiniAES::UCharArrayToQByteArray(unsigned char *src, int p_size)
{
...
// pkcs#7 padding
QByteArray MiniAES::RemovePadding(QByteArray input)
{
...
QByteArray MiniAES::AddPadding(QByteArray input)
{
    int size = input.size();
...
// algorithm
void MiniAES::xor_block( void *d, const void *s )
{
...
void MiniAES::copy_and_key( void *d, const void *s, const void *k )
{
...
void MiniAES::add_round_key( uint_8t d[N_BLOCK], const uint_8t k[N_BLOCK] )
{
    xor_block(d, k);
}

void MiniAES::shift_sub_rows( uint_8t st[N_BLOCK] )
{
    uint_8t tt;
...
void MiniAES::inv_shift_sub_rows( uint_8t st[N_BLOCK] )
{
    uint_8t tt;
...
void MiniAES::mix_sub_columns( uint_8t dt[N_BLOCK] )
{
    uint_8t st[N_BLOCK];
    block_copy(st, dt);
...
void MiniAES::inv_mix_sub_columns( uint_8t dt[N_BLOCK] )
{
    uint_8t st[N_BLOCK];
    block_copy(st, dt);
...
aes_result MiniAES::aes_set_key( const unsigned char key[], int keylen, aes_context ctx[1] )
{
    uint_8t cc, rc, hi;
...
aes_result MiniAES::aes_encrypt( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
...
aes_result MiniAES::aes_cbc_encrypt(const unsigned char *in, unsigned char *out, unsigned long size, unsigned char iv[N_BLOCK], const aes_context ctx[1] )
{
    if (size % 16 != 0)
        return EXIT_FAILURE;
...
aes_result MiniAES::aes_decrypt( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
        uint_8t s1[N_BLOCK], r;
...
aes_result MiniAES::aes_cbc_decrypt( const unsigned char *in, unsigned char *out, unsigned long size, unsigned char iv[N_BLOCK], const aes_context ctx[1] )
{
    if (size % 16 != 0)
        return EXIT_FAILURE;
